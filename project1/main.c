...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        { // first pass though we set a line to the 2nd line to get started
            ...
            { // not the same length so its unique
                char a;
                int stopvar = 0;
                while (stopvar == 0)
                {
...
int checkmatch(int fp)
{
    ...
    char ch1, ch2;
    int tmppos1 = line1pos, tmppos2 = line2pos;
    if (debugvar == 1)
        printf("%d, %d  \n", line1pos, line2pos);
    while (ret == 0)
    {
        // seek to first line, this is new
        lseekcheck2 = lseek(fp, tmppos1, SEEK_SET);
        if (lseekcheck2 == -1) // error check
        {
            perror("Else Problem: ");
            exit(1);
        }
        ...
        lseekcheck1 = lseek(fp, tmppos2, SEEK_SET);
